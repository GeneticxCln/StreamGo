...
async fn verify_builtin_addons_have_urls() {
    let addons = api::get_builtin_addons()
        .await
        .expect("Failed to get builtin addons");
    ...
    for addon in &addons {
        ...
        assert!(
            addon.url.starts_with("http"),
            "Addon '{}' URL doesn't start with http",
            addon.name
        );
        assert_ne!(
            addon.url, "built-in",
            "Addon '{}' has placeholder URL",
            addon.name
        );
    }
